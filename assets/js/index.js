	...
	$('#signInModal').on('shown.bs.modal', function () {
		...
		var $error = $modal.find("#errorinfo");
	 	$modal.on('click','[data-node="confirm"]',function (e) {
	 		...
	 		if (params["username"] === '' || params["password"] === '' || params["verifycode"] === '') {
	 			$error.html('用户信息填写不全!');
	 			return;
	 		}
	 		if (!params["username"].match(/^[a-zA-z][a-zA-Z0-9_]{2,9}$/)) {
	 			$error.html('用户名填写不规范!');
	 			return;
	 		}
	 		if (!params["password"].match(/((?=.*\d)(?=.*\D)|(?=.*[a-zA-Z])(?=.*[^a-zA-Z]))^.{8,16}$/)){
	 			$error.html('密码填写不规范!');
	 			return;
	 		}
	 		if (!params["verifycode"].match(/^[a-zA-Z0-9]{4}$/)){
	 			$error.html('验证码填写不规范!');
	 			return;
	 		}
	 		$.ajax({ 
	 			...
	 			data: {
	 				...
	 				verify: params['verifycode']
	 			},
	 			dataType: "json",
	 			success: function(data) {
	 				if (data.success) { 
	 					$modal.modal('hide');
	 					global.trigger('loginIn');
	 					$signName.html(data.data.username);
	 					toastr.success(data.msg);
	 				} else {
	 					toastr.warning(data.msg);
	 					$modal.find('img').trigger('click');
	 				}  
	 			},
	 			error: function(req){
	...
	$('#signUpModal').on('shown.bs.modal', function () {
		...
		var $error = $modal.find("#errorinfo");
	 	$modal.on('click','[data-node="confirm"]',function (e) {
	 		e.stopPropagation();
	 		var params = getModalParams($modal);
	 		if (params["username"] === '' || params["password"] === '' || params["password-confirm"] === '') {
	 			$error.html('用户信息填写不全!');
	 			return;
	 		}
	 		if (!params["username"].match(/^[a-zA-z][a-zA-Z0-9_]{2,9}$/)) {
